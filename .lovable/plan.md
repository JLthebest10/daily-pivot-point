# Como recriar o Life Hub em outro chat

Você quer criar exatamente este projeto em um novo chat do Lovable, já conectado com o banco de dados (Lovable Cloud / Supabase). O caminho mais confiável é abrir um novo chat e colar um prompt único e completo, com três coisas embutidas:

1. A especificação funcional completa (módulos, layout, comportamentos).
2. O SQL de migração do banco (todas as tabelas, RLS, triggers).
3. As decisões de arquitetura (sem login, design system, hooks de dados).

Como cada chat do Lovable parte de um projeto novo em branco, não dá pra "clonar" diretamente. O que dá pra fazer é entregar ao novo chat um briefing tão detalhado que ele reproduza o projeto na primeira tentativa.

## Passo a passo

1. Crie um novo projeto/chat no Lovable.
2. O Lovable Cloud já vem habilitado por padrão em projetos novos — o banco, auth e storage já estão prontos. Não precisa conectar nada manualmente.
3. Cole o prompt abaixo (incluindo o bloco SQL) como a primeira mensagem.
4. O agente do novo chat vai: aplicar a migração, criar o design system, montar a camada de dados e construir todos os módulos na sequência.
...
=== ARQUITETURA: SEM LOGIN ===
É um app pessoal de um único usuário. NÃO tem tela de login nem cadastro visível. O app cria silenciosamente uma conta vinculada ao dispositivo (credenciais guardadas em localStorage) e mantém a sessão ativa. Criar src/lib/session.ts com a função ensureSession() que:
- Verifica se já há sessão no Supabase; se sim, retorna.
- Se há credenciais salvas em localStorage (chave "lifehub-device-account"), tenta signInWithPassword.
- Senão, gera um email aleatório (hub-{uuid}@lifehub.app) e senha, faz signUp, salva as credenciais em localStorage, e garante a sessão.
O layout _authenticated/route.tsx chama ensureSession() no beforeLoad (ssr: false). A rota "/" redireciona para "/hoje". Não há botão de logout em lugar nenhum.
...
=== HOOKS ===
- useProfile(): lê/cria o profile do usuário atual (tabela profiles). Se não existir, cria automaticamente com o email como name.
- useUpdateProfile(): mutation para atualizar perfil.
- useTheme(): gerencia light/dark/system.

=== LAYOUT (AppShell) ===
Sidebar fixa (w-60) no desktop (lg+), com navegação completa. Header sticky no mobile com botão de busca e tema. Bottom nav no mobile com 5 itens (Hoje, Hábitos, Treino, Calendário) + botão "Mais" que abre um Sheet com todos os módulos. Busca global (GlobalSearch) via Sheet.

Navegação (rotas): /hoje, /habitos, /treino, /financas, /pets, /calendario, /tarefas, /metas, /insights, /configuracoes, /produtividade.
...
1. HOJE (/hoje) — ordem de cima para baixo:
   a. Cabeçalho: saudação (Bom dia/tarde/noite) + data por extenso em português.
...
11. CONFIGURAÇÕES (/configuracoes) — tema, meta semanal de treinos, meta mensal de poupança, nome do perfil.
...
3. Criar src/lib/db.ts (hooks genéricos), src/lib/session.ts (ensureSession), src/lib/format.ts (money, datas em pt-BR, greeting), src/lib/habits.ts (isScheduled, streaks, habitStats).
4. Criar hooks useProfile, useTheme.
5. Criar AppShell com navegação e GlobalSearch.
6. Criar ui-kit.tsx (PageHeader, StatCard, CircularProgress, DayTrack, FormModal, etc).
7. Construir cada módulo na ordem: hoje, habitos, treino, financas, pets, calendario, tarefas, metas, insights, produtividade, configuracoes.
8. Conectar _authenticated/route.tsx com ensureSession no beforeLoad (ssr: false). Rota "/" redireciona para /hoje.
```

## Observações importantes

- O novo chat começará com um projeto limpo e o Lovable Cloud já ativo. O SQL acima é a migração completa e única — ele cria todas as 14 tabelas, RLS, triggers, grants e o bucket de storage.
- O design system (paleta sage/ink em oklch + fonte Instrument Sans) é o que dá a identidade visual ao app. Sem essas cores específicas, o resultado parecerá genérico.
- A arquitetura sem login (ensureSession) é o coração do "app pessoal" — sem ela, o usuário veria telas de auth.
- Os dados NÃO vêm junto: o novo projeto terá um banco novo e vazio. Os hábitos, tarefas e transações que você criou aqui ficam neste projeto. Se quiser migrar os dados também, exporte via Cloud > Advanced settings > Export data neste projeto e me peça para ajudar a importar no novo.
